...
use Dinamiko\DKPDF\Vendor\Mpdf\Shaper\Indic;
use Dinamiko\DKPDF\Vendor\Mpdf\Shaper\Myanmar;
use Dinamiko\DKPDF\Vendor\Mpdf\Shaper\Sea;
...
class Otl
{
	...
	public function __construct($mpdf, FontCache $fontCache)
	{
		$this->mpdf = $mpdf;
	...
	function applyOTL($str, $useOTL)
	{
		...
		for ($sch = 0; $sch <= $subchunk; $sch++) {
			...
			if (!isset($this->GDEFdata[$this->fontkey]['GSUBGPOStables'])) {
				...
				if (!$this->ttfOTLdata) {
					throw new \Dinamiko\DKPDF\Vendor\Mpdf\MpdfException('Can\'t open file ' . $this->fontCache->tempFilename($this->fontkey . '.GSUBGPOStables.dat'));
				}
			} else {
	...
	function _applyGSUBsubtable($lookupID, $subtable, $ptr, $currGlyph, $currGID, $subtable_offset, $Type, $Flag, $MarkFilteringSet, $LuCoverage, $level, $currentTag, $is_old_spec, $tagInt)
	{
		...
		elseif ($Type == 5) {
			...
			elseif ($SubstFormat == 3) {
				throw new \Dinamiko\DKPDF\Vendor\Mpdf\MpdfException("GSUB Lookup Type " . $Type . " Format " . $SubstFormat . " not TESTED YET.");
			}
		} ////////////////////////////////////////////////////////////////////////////////
		...
		elseif ($Type == 6) {
			...
			throw new \Dinamiko\DKPDF\Vendor\Mpdf\MpdfException("GSUB Lookup Type " . $Type . " not supported.");
		}
	}
...
//echo "PROBLEM: ".($x).'<br />';
	...
	private function _applyGPOSsubtable($lookupID, $subtable, $ptr, $currGlyph, $currGID, $subtable_offset, $Type, $Flag, $MarkFilteringSet, $LuCoverage, $tag, $level, $is_old_spec)
	{
		...
		elseif ($Type == 7) {
			...
			//===========
			if ($PosFormat == 1) {
				throw new \Dinamiko\DKPDF\Vendor\Mpdf\MpdfException("GPOS Lookup Type " . $Type . " Format " . $PosFormat . " not TESTED YET.");
			} //===========
			// Format 2:
			//===========
			elseif ($PosFormat == 2) {
			...
			elseif ($PosFormat == 3) {
				...
				throw new \Dinamiko\DKPDF\Vendor\Mpdf\MpdfException("GPOS Lookup Type " . $Type . ", Format " . $PosFormat . " not supported.");
			}
		} ////////////////////////////////////////////////////////////////////////////////
		...
		elseif ($Type == 8) {
			...
			throw new \Dinamiko\DKPDF\Vendor\Mpdf\MpdfException("GPOS Lookup Type " . $Type . " not supported.");
		}
	}
	...
	private function _getGCOMignoreString($flag, $MarkFilteringSet)
	{
		...
		if ($flag & 0x0010) {
			throw new \Dinamiko\DKPDF\Vendor\Mpdf\MpdfException("This font [" . $this->fontkey . "] contains MarkGlyphSets - Not tested yet");
			// Change also in ttfontsuni.php
			if ($MarkFilteringSet == '') {
				throw new \Dinamiko\DKPDF\Vendor\Mpdf\MpdfException("This font [" . $this->fontkey . "] contains MarkGlyphSets - but MarkFilteringSet not set");
			}
