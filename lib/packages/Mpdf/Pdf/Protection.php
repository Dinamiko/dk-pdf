...
namespace Dinamiko\DKPDF\Vendor\Mpdf\Pdf;
...
class Protection
{
	...
	/**
	 * @var \Dinamiko\DKPDF\Vendor\Mpdf\Pdf\Protection\UniqidGenerator
	 */
	...
	public function setProtection($permissions = [], $user_pass = '', $owner_pass = null, $length = 40)
	{
		...
		if ($length === 128) {
			...
			throw new \Dinamiko\DKPDF\Vendor\Mpdf\MpdfException('PDF protection only allows lenghts of 40 or 128');
		}
	...
	private function getProtectionBitsFromOptions($permissions)
	{
		...
		foreach ($permissions as $permission) {
			if (!isset($this->options[$permission])) {
				throw new \Dinamiko\DKPDF\Vendor\Mpdf\MpdfException(sprintf('Invalid permission type "%s"', $permission));
			}
