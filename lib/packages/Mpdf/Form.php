...
class Form
{
	...
	public function __construct($mpdf, Otl $otl, ColorConverter $colorConverter, BaseWriter $writer, FormWriter $formWriter)
	{
		$this->mpdf = $mpdf;
		$this->otl = $otl;
		...
		if (isset($this->pdf_acro_array)) {
			...
			foreach ($this->form_fonts as $fn) {
				if (is_array($this->mpdf->fonts[$fn]['n'])) {
					throw new \Dinamiko\DKPDF\Vendor\Mpdf\MpdfException('Cannot use fonts with SMP or SIP characters for interactive Form elements');
				}
	...
	function SetFormText($w, $h, $name, $value = '', $default = '', $title = '', $flags = [], $align = 'L', $hidden = false, $maxlen = -1, $js = '', $background_col = false, $border_col = false)
	{
		...
		if (!preg_match('/^[a-zA-Z0-9_:\-]+$/', $name)) {
			throw new \Dinamiko\DKPDF\Vendor\Mpdf\MpdfException('Field [' . $name . '] must have a name attribute, which can only contain letters, numbers, colon(:), undersore(_) or hyphen(-)');
		}
	...
	function SetFormButton($bb, $hh, $name, $value, $type, $title = '', $flags = [], $checked = false, $disabled = false, $background_col = false, $border_col = false, $noprint = false)
	{
		...
		if ($type === 'radio' || $type === 'checkbox') {
			if (!preg_match('/^[a-zA-Z0-9_:\-\.]+$/', $value)) {
				throw new \Dinamiko\DKPDF\Vendor\Mpdf\MpdfException("Field '" . $name . "' must have a value, which can only contain letters, numbers, colon(:), underscore(_), hyphen(-) or period(.)");
			}
		}
		if ($type === 'radio') {
	...
	function _put_button_icon($array, $w, $h)
	{
		...
		if (!$info) {
			throw new \Dinamiko\DKPDF\Vendor\Mpdf\MpdfException('Cannot find Button image');
		}
