...
namespace Dinamiko\DKPDF\Vendor\Mpdf\Image;

use Dinamiko\DKPDF\Vendor\Mpdf\AssetFetcher;
...
use Dinamiko\DKPDF\Vendor\Mpdf\Gif\Gif;
use Dinamiko\DKPDF\Vendor\Mpdf\Language\LanguageToFontInterface;
use Dinamiko\DKPDF\Vendor\Mpdf\Language\ScriptToLanguageInterface;
...
class ImageProcessor implements \Dinamiko\DKPDF\Vendor\Psr\Log\LoggerAwareInterface
{
	...
	/**
	 * @var \Dinamiko\DKPDF\Vendor\Mpdf\Image\ImageTypeGuesser
	 */
	private $guesser;
	...
	public function getImage(&$file, $firstTime = true, $allowvector = true, $orig_srcpath = false, $interpolation = false)
	{
		...
		if (!$data) {
			...
			} catch (\Dinamiko\DKPDF\Vendor\Mpdf\Exception\AssetFetchingException $e) {
	...
	private function gzCompress($data)
	{
		if (!function_exists('gzcompress')) {
			throw new \Dinamiko\DKPDF\Vendor\Mpdf\MpdfException('gzcompress is not available. install ext-zlib extension.');
		}
	...
	private function imageError($file, $firstTime, $msg)
	{
		...
		if ($firstTime && ($this->mpdf->showImageErrors || $this->mpdf->debug)) {
			throw new \Dinamiko\DKPDF\Vendor\Mpdf\MpdfImageException(sprintf('%s (%s)', $msg, substr($file, 0, 256)));
		}
	...
	public function processJpg($data, $file, $firstTime, $interpolation)
	{
		...
		if ($a[2] === 'DeviceCMYK' && ($this->mpdf->restrictColorSpace === 2 || ($this->mpdf->PDFA && $this->mpdf->restrictColorSpace !== 3))) {
			...
			if (!function_exists('gd_info')) {
				throw new \Dinamiko\DKPDF\Vendor\Mpdf\MpdfException(sprintf('JPG image may not use CMYK color space (%s).', $file));
			}
	...
	public function processPng($data, $file, $firstTime, $interpolation)
	{
		...
		if ($firstTime && ($colspace === 'DeviceRGB' || $colspace === 'Indexed') && ($this->mpdf->PDFX || $this->mpdf->restrictColorSpace === 3)) {
			...
			if ($pngalpha) {
				if ($this->mpdf->PDFA) {
					throw new \Dinamiko\DKPDF\Vendor\Mpdf\MpdfException(sprintf('PDFA1-b does not permit images with alpha channel transparency (%s).', $file));
				}
