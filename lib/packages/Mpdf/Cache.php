...
class Cache
{
	...
	public function __construct($basePath, $cleanupInterval = 3600)
	{
		if (!is_int($cleanupInterval) && false !== $cleanupInterval) {
			throw new \Dinamiko\DKPDF\Vendor\Mpdf\MpdfException('Cache cleanup interval has to be an integer or false');
		}

		if (!$this->createBasePath($basePath)) {
			throw new \Dinamiko\DKPDF\Vendor\Mpdf\MpdfException(sprintf('Temporary files directory "%s" is not writable', $basePath));
		}
