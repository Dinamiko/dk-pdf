...
namespace Dinamiko\DKPDF\Vendor\Mpdf\Color;
...
class ColorConverter
{
	...
	public function invert($c)
	{
		...
		throw new \Dinamiko\DKPDF\Vendor\Mpdf\MpdfException('Trying to invert non-RGB color');
	}
			...
			case 'spot':
				...
				if (!isset($this->mpdf->spotColors[$name])) {
					if (isset($cores[5])) {
						...
						throw new \Dinamiko\DKPDF\Vendor\Mpdf\MpdfException(sprintf('Undefined spot color "%s"', $name));
					}
				}
	...
	{
		if (!is_string($color)) {
			throw new \Dinamiko\DKPDF\Vendor\Mpdf\MpdfException('Invalid color input, binary color string expected');
		}
		...
		if (!in_array($color[0], [static::MODE_GRAYSCALE, static::MODE_SPOT, static::MODE_RGB, static::MODE_CMYK, static::MODE_RGBA, static::MODE_CMYKA])) {
			throw new \Dinamiko\DKPDF\Vendor\Mpdf\MpdfException('Invalid color input, invalid color mode in binary color string');
		}
	}
