...
use Dinamiko\DKPDF\Vendor\Mpdf\Fonts\GlyphOperator;
...
// mPDF 5.7.1
if (!function_exists('\Dinamiko\DKPDF\Vendor\Mpdf\unicode_hex')) {
...
class TTFontFile
{
	...
	public function getMetrics($file, $fontkey, $TTCfontID = 0, $debug = false, $BMPonly = false, $useOTL = 0)
	{
		...
		if (!$this->fh) {
			throw new \Dinamiko\DKPDF\Vendor\Mpdf\Exception\FontException(sprintf('Unable to open font file "%s"', $file));
		}
		...
		if ($version === 0x4F54544F) {
			throw new \Dinamiko\DKPDF\Vendor\Mpdf\Exception\FontException(sprintf('Fonts with postscript outlines are not supported (%s)', $file));
		}

		if ($version === 0x74746366 && !$TTCfontID) {
			throw new \Dinamiko\DKPDF\Vendor\Mpdf\Exception\FontException(sprintf('TTCfontID for a TrueType Collection is not defined in mPDF "fontdata" configuration (%s)', $file));
		}

		if (!in_array($version, [0x00010000, 0x74727565], true) && !$TTCfontID) {
			throw new \Dinamiko\DKPDF\Vendor\Mpdf\Exception\FontException(sprintf('Not a TrueType font: version=%s)', $version));
		}

		if ($TTCfontID > 0) {
			$this->version = $version = $this->read_ulong(); // TTC Header version now
			if (!in_array($version, [0x00010000, 0x00020000], true)) {
				throw new \Dinamiko\DKPDF\Vendor\Mpdf\Exception\FontException(sprintf('Error parsing TrueType Collection: version=%s - (%s)', $version, $file));
			}
		...
		foreach ($this->tables as $t) {
			if ($t['length'] > 0 && $t['length'] < $this->maxStrLenRead) { // 1.02
				...
				if ($xchecksum != $checksum) {
					throw new \Dinamiko\DKPDF\Vendor\Mpdf\Exception\FontException(sprintf('TTF file "%s": invalid checksum %s table: %s (expected %s)', $this->filename, dechex($checksum[0]) . dechex($checksum[1]), $t['tag'], dechex($xchecksum[0]) . dechex($xchecksum[1])));
				}
			}
		}
	}
	...
	function getCTG($file, $TTCfontID = 0, $debug = false, $useOTL = false)
	{
		...
		if (!$this->fh) {
			throw new \Dinamiko\DKPDF\Vendor\Mpdf\Exception\FontException(sprintf('Unable to open file "%s"', $file));
		}
		...
		if ($useOTL) {
			...
			for ($gid = 1; $gid < $numGlyphs; $gid++) {
				if (!isset($glyphToChar[$gid])) {
					...
					if ($bctr > 0xF8FF) {
						throw new \Dinamiko\DKPDF\Vendor\Mpdf\Exception\FontException(sprintf('Font "%s" cannot map all included glyphs into Private Use Area U+E000-U+F8FF; cannot use useOTL on this font', $file));
					}
	...
	function getTTCFonts($file)
	{
		...
		if ($version === 0x74746366) {
			...
			throw new \Dinamiko\DKPDF\Vendor\Mpdf\Exception\FontException(sprintf("Not a TrueType Collection: version=%s (%s)", $version, $file));
		}
	...
	function extractInfo($debug = false, $BMPonly = false, $useOTL = 0)
	{
		...
		if ($format != 0 && $format != 1) {
			throw new \Dinamiko\DKPDF\Vendor\Mpdf\Exception\FontException("Error loading font: Unknown name table format $format for font $this->filename");
		}
		...
		for ($i = 0; $i < $numRecords; $i++) {
			...
			if ($platformId == 3 && $encodingId == 1 && $languageId == 0x409) { // Microsoft, Unicode, US English, PS Name
				...
				if ($length % 2 != 0) {
					throw new \Dinamiko\DKPDF\Vendor\Mpdf\Exception\FontException("Error loading font: PostScript name is UTF-16BE string of odd length for font $this->filename");
				}
				$length /= 2;
				$N = '';
				while ($length > 0) {
		...
		if (!$psName) {
			throw new \Dinamiko\DKPDF\Vendor\Mpdf\Exception\FontException("Error loading font: Could not find PostScript font name '$this->filename'");
		}
		...
		for ($i = 0; $i < $nameLength; $i++) {
			...
			if ($oc > 126 || strpos(' [](){}<>/%', $c) !== false) {
				//throw new \Dinamiko\DKPDF\Vendor\Mpdf\Exception\FontException("psName=".$psName." contains invalid character ".$c." ie U+".ord(c));
				$psNameInvalid = true;
				break;
			}
		}
		...
		if ($debug) {
			...
			if ($ver_maj != 1) {
				throw new \Dinamiko\DKPDF\Vendor\Mpdf\Exception\FontException('Error loading font: Unknown head table version ' . $ver_maj . '.' . $ver_min);
			}
			...
			if ($magic !== 0x5F0F3CF5) {
				throw new \Dinamiko\DKPDF\Vendor\Mpdf\Exception\FontException('Error loading font: Invalid head table magic ' . $magic);
			}
			$this->skip(2);
		} else {
			$this->skip(18);
		...
		if ($glyphDataFormat != 0) {
			throw new \Dinamiko\DKPDF\Vendor\Mpdf\Exception\FontException(sprintf('Error loading font: Unknown glyph data format %s', $glyphDataFormat));
		}
		...
		if ($debug) {
			...
			if ($ver_maj < 1 || $ver_maj > 4) {
				throw new \Dinamiko\DKPDF\Vendor\Mpdf\Exception\FontException(sprintf('Error loading font: Unknown post table version %s', $ver_maj));
			}
		} else {
			$this->skip(4);
		}
		...
		if ($debug) {
			$ver_maj = $this->read_ushort();
			if ($ver_maj != 1) {
				throw new \Dinamiko\DKPDF\Vendor\Mpdf\Exception\FontException(sprintf('Error loading font: Unknown hhea table version %s', $ver_maj));
			}
			$this->skip(28);
		} else {
			$this->skip(32);
		...
		if ($metricDataFormat != 0) {
			throw new \Dinamiko\DKPDF\Vendor\Mpdf\Exception\FontException(sprintf('Error loading font: Unknown horizontal metric data format "%s"', $metricDataFormat));
		}

		$numberOfHMetrics = $this->read_ushort();

		if ($numberOfHMetrics == 0) {
			throw new \Dinamiko\DKPDF\Vendor\Mpdf\Exception\FontException('Error loading font: Number of horizontal metrics is 0');
		}
		...
		if ($debug) {
			$ver_maj = $this->read_ushort();
			if ($ver_maj != 1) {
				throw new \Dinamiko\DKPDF\Vendor\Mpdf\Exception\FontException(sprintf('Error loading font: Unknown maxp table version %s', $ver_maj));
			}
		} else {
			$this->skip(4);
		}
		...
		if (!$unicode_cmap_offset) {
			throw new \Dinamiko\DKPDF\Vendor\Mpdf\Exception\FontException(sprintf('Font "%s" does not have cmap for Unicode (platform 3, encoding 1, format 4, or platform 0, any encoding, format 4)', $this->filename));
		}
				...
				if (!isset($glyphToChar[$gid])) {
					...
					if (($bctr > 0xF8FF) && ($bctr < 0x2CEB0)) {
						if (!$BMPonly) {
							...
							throw new \Dinamiko\DKPDF\Vendor\Mpdf\Exception\FontException(sprintf('The font "%s" does not have enough space to map all (unmapped) included glyphs into Private Use Area U+E000-U+F8FF', $names[1]));
						}
					}
	...
	function _getGDEFtables()
	{
		...
		if (isset($this->tables["GDEF"])) {
			...
			throw new \Dinamiko\DKPDF\Vendor\Mpdf\Exception\FontException(sprintf('Unable to set font "%s" to use OTL as it does not include OTL tables (or at least not a GDEF table).', $this->filename));
		}
			...
			for ($c = 0; $c < $Lookup[$i]['SubtableCount']; $c++) {
				...
				elseif ($Lookup[$i]['Type'] == 5) {
					...
					elseif ($SubstFormat == 2) {
						...
						throw new \Dinamiko\DKPDF\Vendor\Mpdf\Exception\FontException("GPOS Lookup Type " . $Lookup[$i]['Type'] . ", Format " . $SubstFormat . " not supported (ttfontsuni.php).");
					}
				...
				elseif ($Lookup[$i]['Type'] == 6) {
					...
					throw new \Dinamiko\DKPDF\Vendor\Mpdf\Exception\FontException(sprintf('Lookup Type "%s" not supported.', $Lookup[$i]['Type']));
				}
			}
		}
			...
			for ($c = 0; $c < $Lookup[$i]['SubtableCount']; $c++) {
				...
				elseif ($Lookup[$i]['Type'] == 5) {
					...
					elseif ($SubstFormat == 3) {
						...
						throw new \Dinamiko\DKPDF\Vendor\Mpdf\Exception\FontException("Lookup Type 5, SubstFormat 3 not tested. Please report this with the name of font used - " . $this->fontkey);
					}
				} // LookupType 6: Chaining Contextual Substitution Subtable
				elseif ($Lookup[$i]['Type'] == 6) {
					// Format 1: Simple Chaining Context Glyph Substitution  p255
		...
		foreach ($gsub as $st => $scripts) {
			foreach ($scripts as $t => $langsys) {
				...
				if (strpos('arab syrc hebr thaa nko  samr', $st) !== false) { // all RTL scripts [any/all languages] ? Mandaic
					...
					// For kashida, need to determine all final forms except ones already identified by kashida priority rules (see \Dinamiko\DKPDF\Vendor\Mpdf\Otl)
	...
	function _getGSUBignoreString($flag, $MarkFilteringSet)
	{
		...
		// Flag & 0x0010 = UseMarkFilteringSet
		if ($flag & 0x0010) {
			throw new \Dinamiko\DKPDF\Vendor\Mpdf\Exception\FontException("Font \"" . $this->fontkey . "\" contains MarkGlyphSets which is not supported");
	...
	function makeSubset($file, &$subset, $TTCfontID = 0, $debug = false, $useOTL = false)
	{
		...
		if (!$unicode_cmap_offset) {
			throw new \Dinamiko\DKPDF\Vendor\Mpdf\Exception\FontException(sprintf('Font "%s" does not have Unicode cmap (platform 3, encoding 1, format 4, or platform 0 [any encoding] format 4)', $this->filename));
		}
		...
		if ($useOTL) {
			...
			for ($gid = 1; $gid < $numGlyphs; $gid++) {
				if (!isset($glyphToChar[$gid])) {
					...
					if ($bctr > 0xF8FF) {
						throw new \Dinamiko\DKPDF\Vendor\Mpdf\Exception\FontException($file . " : WARNING - Font cannot map all included glyphs into Private Use Area U+E000 - U+F8FF; cannot use useOTL on this font");
					}
					$glyphToChar[$gid][] = $bctr;
					$charToGlyph[$bctr] = $gid;
					$bctr++;
	...
	function makeSubsetSIP($file, &$subset, $TTCfontID = 0, $debug = false, $useOTL = 0)
	{
		...
		if ($TTCfontID > 0) {
			...
			if (!in_array($version, [0x00010000, 0x00020000])) {
				throw new \Dinamiko\DKPDF\Vendor\Mpdf\Exception\FontException("ERROR - Error parsing TrueType Collection: version=" . $version . " - " . $file);
			}
			$this->numTTCFonts = $this->read_ulong();
			for ($i = 1; $i <= $this->numTTCFonts; $i++) {
				$this->TTCFonts[$i]['offset'] = $this->read_ulong();
		...
		if (!$unicode_cmap_offset) {
			throw new \Dinamiko\DKPDF\Vendor\Mpdf\Exception\FontException(sprintf('Font "%s" does not have cmap for Unicode (platform 3, encoding 1, format 4, or platform 0, any encoding, format 4)', $file));
		}
	...
	function getLOCA($indexToLocFormat, $numGlyphs)
	{
		...
		if ($indexToLocFormat == 0) {
			...
			throw new \Dinamiko\DKPDF\Vendor\Mpdf\Exception\FontException('Unknown location table format ' . $indexToLocFormat);
		}
	}
		...
		if ($useOTL) {
			...
			for ($gid = 1; $gid < $numGlyphs; $gid++) {
				if (!isset($glyphToChar[$gid])) {
					...
					if ($bctr > 0xF8FF) {
						throw new \Dinamiko\DKPDF\Vendor\Mpdf\Exception\FontException("Problem. Trying to repackage TF file; not enough space for unmapped glyphs");
					}
					$glyphToChar[$gid][] = $bctr;
					$charToGlyph[$bctr] = $gid;
					$bctr++;
