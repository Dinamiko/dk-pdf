...
class DirectWrite
{
	...
	public function __construct($mpdf, Otl $otl, SizeConverter $sizeConverter, ColorConverter $colorConverter)
	{
		$this->mpdf = $mpdf;
		$this->otl = $otl;
	...
	function CircularText($x, $y, $r, $text, $align = 'top', $fontfamily = '', $fontsizePt = 0, $fontstyle = '', $kerning = 120, $fontwidth = 100, $divider = '')
	{
		...
		if ($kerning == 0) {
			throw new \Dinamiko\DKPDF\Vendor\Mpdf\MpdfException('Please use values unequal to zero for kerning (CircularText)');
		}

		if ($fontwidth == 0) {
			throw new \Dinamiko\DKPDF\Vendor\Mpdf\MpdfException('Please use values unequal to zero for font width (CircularText)');
		}
