...
use Dinamiko\DKPDF\Vendor\Mpdf\Conversion;
use Dinamiko\DKPDF\Vendor\Mpdf\Css\Border;
use Dinamiko\DKPDF\Vendor\Mpdf\Css\TextVars;
use Dinamiko\DKPDF\Vendor\Mpdf\Log\Context as LogContext;
use Dinamiko\DKPDF\Vendor\Mpdf\Fonts\MetricsGenerator;
use Dinamiko\DKPDF\Vendor\Mpdf\Output\Destination;
use Dinamiko\DKPDF\Vendor\Mpdf\PsrLogAwareTrait\MpdfPsrLogAwareTrait;
use Dinamiko\DKPDF\Vendor\Mpdf\QrCode;
use Dinamiko\DKPDF\Vendor\Mpdf\Utils\Arrays;
use Dinamiko\DKPDF\Vendor\Mpdf\Utils\NumericString;
use Dinamiko\DKPDF\Vendor\Mpdf\Utils\UtfString;
use Dinamiko\DKPDF\Vendor\Psr\Log\NullLogger;
 ...
 * @license GPL-2.0
 */
class Mpdf implements \Dinamiko\DKPDF\Vendor\Psr\Log\LoggerAwareInterface
{
	...
	/**
	 * @var \Dinamiko\DKPDF\Vendor\Mpdf\Gradient
	 */
	private $gradient;

	/**
	 * @var \Dinamiko\DKPDF\Vendor\Mpdf\Image\Bmp
	 */
	private $bmp;

	/**
	 * @var \Dinamiko\DKPDF\Vendor\Mpdf\Image\Wmf
	 */
	private $wmf;
	...
	/**
	 * @var \Dinamiko\DKPDF\Vendor\Mpdf\DirectWrite
	 */
	private $directWrite;
	...
	/**
	 * @var \Dinamiko\DKPDF\Vendor\Mpdf\Fonts\FontCache
	 */
	private $fontCache;

	/**
	 * @var \Dinamiko\DKPDF\Vendor\Mpdf\Fonts\FontFileFinder
	 */
	private $fontFileFinder;

	/**
	 * @var \Dinamiko\DKPDF\Vendor\Mpdf\Tag
	 */
	private $tag;

	/**
	 * @var \Dinamiko\DKPDF\Vendor\Mpdf\Barcode
	 * @todo solve Tag dependency and make private
	 */
	public $barcode;

	/**
	 * @var \Dinamiko\DKPDF\Vendor\Mpdf\QrCode\QrCode
	 */
	private $qrcode;
	...
	/**
	 * @var \Dinamiko\DKPDF\Vendor\Mpdf\Color\ColorModeConverter
	 */
	private $colorModeConverter;

	/**
	 * @var \Dinamiko\DKPDF\Vendor\Mpdf\Color\ColorSpaceRestrictor
	 */
	private $colorSpaceRestrictor;

	/**
	 * @var \Dinamiko\DKPDF\Vendor\Mpdf\Hyphenator
	 */
	private $hyphenator;

	/**
	 * @var \Dinamiko\DKPDF\Vendor\Mpdf\Pdf\Protection
	 */
	private $protection;

	/**
	 * @var \Dinamiko\DKPDF\Vendor\Mpdf\Http\ClientInterface
	 */
	private $httpClient;

	/**
	 * @var \Dinamiko\DKPDF\Vendor\Mpdf\File\LocalContentLoaderInterface
	 */
	private $localContentLoader;

	/**
	 * @var \Dinamiko\DKPDF\Vendor\Mpdf\AssetFetcher
	 */
	...
	/**
	 * @var \Dinamiko\DKPDF\Vendor\Mpdf\Language\ScriptToLanguageInterface
	 */
	...
	/**
	 * @var \Dinamiko\DKPDF\Vendor\Mpdf\Writer\FontWriter
	 */
	private $fontWriter;

	/**
	 * @var \Dinamiko\DKPDF\Vendor\Mpdf\Writer\MetadataWriter
	 */
	private $metadataWriter;

	/**
	 * @var \Dinamiko\DKPDF\Vendor\Mpdf\Writer\ImageWriter
	 */
	private $imageWriter;

	/**
	 * @var \Dinamiko\DKPDF\Vendor\Mpdf\Writer\FormWriter
	 */
	private $formWriter;

	/**
	 * @var \Dinamiko\DKPDF\Vendor\Mpdf\Writer\PageWriter
	 */
	private $pageWriter;

	/**
	 * @var \Dinamiko\DKPDF\Vendor\Mpdf\Writer\BookmarkWriter
	 */
	private $bookmarkWriter;

	/**
	 * @var \Dinamiko\DKPDF\Vendor\Mpdf\Writer\OptionalContentWriter
	 */
	private $optionalContentWriter;

	/**
	 * @var \Dinamiko\DKPDF\Vendor\Mpdf\Writer\ColorWriter
	 */
	private $colorWriter;

	/**
	 * @var \Dinamiko\DKPDF\Vendor\Mpdf\Writer\BackgroundWriter
	 */
	private $backgroundWriter;

	/**
	 * @var \Dinamiko\DKPDF\Vendor\Mpdf\Writer\JavaScriptWriter
	 */
	private $javaScriptWriter;

	/**
	 * @var \Dinamiko\DKPDF\Vendor\Mpdf\Writer\ResourceWriter
	 */
	...
	/**
	 * @var \Dinamiko\DKPDF\Vendor\Mpdf\Container\ContainerInterface
	 */
	private $container;

	/**
	 * @param mixed[] $config
	 * @param \Dinamiko\DKPDF\Vendor\Mpdf\Container\ContainerInterface|null $container Experimental container to override internal services
	 */
	public function __construct(array $config = [], $container = null)
	{
		$this->_dochecks();

		assert(!$container || $container instanceof \Dinamiko\DKPDF\Vendor\Mpdf\Container\ContainerInterface);
		...
		if (file_exists($this->defaultCssFile)) {
			...
			throw new \Dinamiko\DKPDF\Vendor\Mpdf\MpdfException(sprintf('Unable to read default CSS file "%s"', $this->defaultCssFile));
		}
	...
	function _setPageSize($format, &$orientation)
	{
		if (is_string($format)) {
			...
			if (!$format[0] || !$format[1]) {
				throw new \Dinamiko\DKPDF\Vendor\Mpdf\MpdfException('Invalid page format: ' . $format[0] . ' ' . $format[1]);
			}
		...
		if ($orientation === 'p' || $orientation === 'portrait') {
			...
			throw new \Dinamiko\DKPDF\Vendor\Mpdf\MpdfException('Incorrect orientation: ' . $orientation);
		}
	...
	function SetDisplayMode($zoom, $layout = 'continuous')
	{
		...
		if (in_array($zoom, $allowedZoomModes, true) || is_numeric($zoom)) {
			...
			throw new \Dinamiko\DKPDF\Vendor\Mpdf\MpdfException('Incorrect zoom display mode: ' . $zoom);
		}
		...
		if (in_array($layout, $allowedLayoutModes, true)) {
			...
			throw new \Dinamiko\DKPDF\Vendor\Mpdf\MpdfException('Incorrect layout display mode: ' . $layout);
		}
	}
	...
	function SetVisibility($v)
	{
		...
		if ($v == 'printonly') {
			...
			throw new \Dinamiko\DKPDF\Vendor\Mpdf\MpdfException('Incorrect visibility: ' . $v);
		}
		$this->visibility = $v;
	}
	...
	function AddSpotColorsFromFile($file)
	{
		$colors = @file($file);
		if (!$colors) {
			throw new \Dinamiko\DKPDF\Vendor\Mpdf\MpdfException("Cannot load spot colors file - " . $file);
		}
	...
	function AddFont($family, $style = '')
	{
		...
		if ($this->usingCoreFont) {
			throw new \Dinamiko\DKPDF\Vendor\Mpdf\MpdfException("mPDF Error - problem with Font management");
		}
		...
		if (!isset($this->fontdata[$family][$stylekey]) || !$this->fontdata[$family][$stylekey]) {
			throw new \Dinamiko\DKPDF\Vendor\Mpdf\MpdfException(sprintf('Font "%s%s%s" is not supported', $family, $style ? ' - ' : '', $style));
		}
	...
	function SetFont($family, $style = '', $size = 0, $write = true, $forcewrite = false)
	{
		...
		if ($family == '') {
			if ($this->FontFamily) {
				...
				throw new \Dinamiko\DKPDF\Vendor\Mpdf\MpdfException("No font or default font set!");
			}
		}
		...
		if (($family == 'csymbol') || ($family == 'czapfdingbats') || ($family == 'ctimes') || ($family == 'ccourier') || ($family == 'chelvetica')) {
			if ($this->PDFA || $this->PDFX) {
				if ($family == 'csymbol' || $family == 'czapfdingbats') {
					throw new \Dinamiko\DKPDF\Vendor\Mpdf\MpdfException("Symbol and Zapfdingbats cannot be embedded in mPDF (required for PDFA1-b or PDFX/1-a).");
				}
		...
		if (!$this->onlyCoreFonts && !$this->usingCoreFont) {
			...
			if ($this->PDFA || $this->PDFX) {
				throw new \Dinamiko\DKPDF\Vendor\Mpdf\MpdfException('Core Adobe fonts cannot be embedded in mPDF (required for PDFA1-b or PDFX/1-a) - cannot use option to use core fonts.');
			}
			...
			if (!isset($this->fonts[$fontkey])) {
				...
				if (isset($this->CoreFonts[$fontkey])) {
					...
					if (!isset($cw)) {
						throw new \Dinamiko\DKPDF\Vendor\Mpdf\MpdfException(sprintf('Could not include font metric file "%s"', $file));
					}
					$i = count($this->fonts) + $this->extraFontSubsets + 1;
					...
					throw new \Dinamiko\DKPDF\Vendor\Mpdf\MpdfException(sprintf('Font %s not defined', $fontkey));
				}
			}
		...
		foreach ($this->objectbuffer as $ib => $objattr) {
			...
			if ($objattr['type'] === 'barcode') {
				...
				if (in_array($objattr['btype'], $barcodeTypes, true)) {
					...
					if (!class_exists('Dinamiko\DKPDF\Vendor\Mpdf\QrCode\QrCode') || !class_exists('Dinamiko\DKPDF\Vendor\Mpdf\QrCode\Output\Mpdf')) {
						throw new \Dinamiko\DKPDF\Vendor\Mpdf\MpdfException('Dinamiko\DKPDF\Vendor\Mpdf\QrCode package was not found. Install the package from Packagist with "composer require mpdf/qrcode"');
					}
	...
	function _SetTextRendering($mode)
	{
		if (!(($mode == 0) || ($mode == 1) || ($mode == 2))) {
			throw new \Dinamiko\DKPDF\Vendor\Mpdf\MpdfException("Text rendering mode should be 0, 1 or 2 (value : $mode)");
		}
		$tr = ($mode . ' Tr');
	...
	function Output($name = '', $dest = '')
	{
		...
		if ($this->debug && error_get_last()) {
			...
			if (($e['type'] < 2048 && $e['type'] != 8) || (intval($e['type']) & intval(ini_get("error_reporting")))) {
				throw new \Dinamiko\DKPDF\Vendor\Mpdf\MpdfException(
		...
		if (($this->PDFA || $this->PDFX) && $this->encrypted) {
			throw new \Dinamiko\DKPDF\Vendor\Mpdf\MpdfException('PDF/A1-b or PDF/X1-a does not permit encryption of documents.');
		}
		...
		if (count($this->PDFAXwarnings) && (($this->PDFA && !$this->PDFAauto) || ($this->PDFX && !$this->PDFXauto))) {
			...
			throw new \Dinamiko\DKPDF\Vendor\Mpdf\MpdfException('PDFA/PDFX warnings generated. See log for further details');
		}
		...
		switch ($dest) {
			...
			case Destination::INLINE:
				...
				if ($this->debug && !$this->allow_output_buffering && ob_get_contents()) {
					throw new \Dinamiko\DKPDF\Vendor\Mpdf\MpdfException('Output has already been sent from the script - PDF file generation aborted.');
				}
			...
			case Destination::DOWNLOAD:
				...
				if (headers_sent()) {
					throw new \Dinamiko\DKPDF\Vendor\Mpdf\MpdfException('Data has already been sent to output, unable to output PDF file');
				}
			...
			case Destination::FILE:
				...
				if (!$f) {
					throw new \Dinamiko\DKPDF\Vendor\Mpdf\MpdfException(sprintf('Unable to create output file %s', $name));
				}
			...
			default:
				throw new \Dinamiko\DKPDF\Vendor\Mpdf\MpdfException(sprintf('Incorrect output destination %s', $dest));
		}
	...
	function _dochecks()
	{
		...
		if (1.1 == 1) {
			throw new \Dinamiko\DKPDF\Vendor\Mpdf\MpdfException('Do not alter the locale before including mPDF');
		}
		...
		if (ini_get('mbstring.func_overload')) {
			throw new \Dinamiko\DKPDF\Vendor\Mpdf\MpdfException('Dinamiko\DKPDF\Vendor\Mpdf cannot function properly with mbstring.func_overload enabled');
		}

		if (!function_exists('mb_substr')) {
			throw new \Dinamiko\DKPDF\Vendor\Mpdf\MpdfException('mbstring extension must be loaded in order to run mPDF');
		}
		...
		if (!function_exists('mb_regex_encoding')) {
			...
			throw new \Dinamiko\DKPDF\Vendor\Mpdf\MpdfException('mbstring extension with mbregex support must be loaded in order to run mPDF.' . $mamp);
		}
	}
	...
	{
		if ($this->PDFA || $this->PDFX) {
			throw new \Dinamiko\DKPDF\Vendor\Mpdf\MpdfException('PDFA and PDFX do not permit transparency, so mPDF does not allow Watermarks!');
		}
	...
	function AddCIDFont($family, $style, $name, &$cw, $CMap, $registry, $desc)
	{
		...
		if (isset($this->fonts[$fontkey])) {
			throw new \Dinamiko\DKPDF\Vendor\Mpdf\MpdfException("Font already added: $family $style");
		}
		$i = count($this->fonts) + $this->extraFontSubsets + 1;
	...
	function AddCJKFont($family)
	{
		...
		if ($this->PDFA || $this->PDFX) {
			throw new \Dinamiko\DKPDF\Vendor\Mpdf\MpdfException("Adobe CJK fonts cannot be embedded in mPDF (required for PDFA1-b and PDFX/1-a).");
		}
		if ($family == 'big5') {
			$this->AddBig5Font();
		} elseif ($family == 'gb') {
	...
	function TableHeaderFooter($content = '', $tablestartpage = '', $tablestartcolumn = '', $horf = 'H', $level = 0, $firstSpread = true, $finalSpread = true)
	{
		if (($horf == 'H' || $horf == 'F') && !empty($content)) { // mPDF 5.7.2
			...
			if (empty($content[$firstrow])) {
				if ($this->debug) {
					throw new \Dinamiko\DKPDF\Vendor\Mpdf\MpdfException("<tfoot> must precede <tbody> in a table");
				} else {
					return;
				}
			}
	...
	{
		if ($txt instanceof \Dinamiko\DKPDF\Vendor\Mpdf\WatermarkText) {
	...
	{
		if ($src instanceof \Dinamiko\DKPDF\Vendor\Mpdf\WatermarkImage) {
	...
	function WriteHTML($html, $mode = HTMLParserMode::DEFAULT_MODE, $init = true, $close = true)
	{
		...
		if (is_scalar($html) === false) {
			if (!is_object($html) || ! method_exists($html, '__toString')) {
				throw new \Dinamiko\DKPDF\Vendor\Mpdf\MpdfException('WriteHTML() requires $html be an integer, float, string, boolean or an object with the __toString() magic method.');
			}
		}

		// Check the mode is valid
		if (in_array($mode, HTMLParserMode::getAllModes(), true) === false) {
			throw new \Dinamiko\DKPDF\Vendor\Mpdf\MpdfException('WriteHTML() requires $mode to be one of the modes defined in HTMLParserMode');
		}
	...
	function printbuffer($arrayaux, $blockstate = 0, $is_table = false, $table_draft = false, $cell_dir = '')
	{
		...
		for ($i = 0; $i < $array_size; $i++) {
			...
			if (($blockdir == 'rtl' || $this->biDirectional) && isset($arrayaux[$i][4]) && in_array($arrayaux[$i][4], ['ccourier', 'ctimes', 'chelvetica', 'csymbol', 'czapfdingbats'])) {
				throw new \Dinamiko\DKPDF\Vendor\Mpdf\MpdfException("You cannot use core fonts in a document which contains RTL text.");
			}
		}
		// mPDF 6
	...
	// mPDF 6
		...
		if (($toonarrow) && ($this->table_error_report)) {
			throw new \Dinamiko\DKPDF\Vendor\Mpdf\MpdfException("Word is too long to fit in table - " . $this->table_error_report_param);
		}
		if ($toonarrow) {
			return -$biggestword;
		} else {
		...
		if (!$this->is_utf8($html)) {
			...
			while (mb_convert_encoding(mb_convert_encoding($html, "UTF-32", "UTF-8"), "UTF-8", "UTF-32") !== $html) {
				...
				if ($error && $error['message'] === 'iconv(): Detected an illegal character in input string') {
					throw new \Dinamiko\DKPDF\Vendor\Mpdf\MpdfException('Invalid input characters. Did you set $mpdf->in_charset properly?');
				}
			...
			throw new \Dinamiko\DKPDF\Vendor\Mpdf\MpdfException("HTML contains invalid UTF-8 character(s). See log for further details");
		}
	...
	function purify_utf8_text($txt)
	{
		...
		if (!$this->is_utf8($txt)) {
			throw new \Dinamiko\DKPDF\Vendor\Mpdf\MpdfException("Text contains invalid UTF-8 character(s)");
		}
	...
	function WriteBarcode($code, $showtext = 1, $x = '', $y = '', $size = 1, $border = 0, $paddingL = 1, $paddingR = 1, $paddingT = 2, $paddingB = 2, $height = 1, $bgcol = false, $col = false, $btype = 'ISBN', $supplement = '0', $supplement_code = '', $k = 1)
	{
		...
		if ($arrcode === false) {
			throw new \Dinamiko\DKPDF\Vendor\Mpdf\MpdfException('Error in barcode string: ' . $codestr);
		}
		...
		if ($supplement) { // EAN-2 or -5 Supplement
			...
			if ($supparrcode === false) {
				throw new \Dinamiko\DKPDF\Vendor\Mpdf\MpdfException('Error in barcode string (supplement): ' . $codestr . ' ' . $supplement_code);
			}

			if (strlen($supplement_code) != $supplement) {
				throw new \Dinamiko\DKPDF\Vendor\Mpdf\MpdfException('Barcode supplement incorrect: ' . $supplement_code);
			}
	...
	function transformScale($s_x, $s_y, $x = '', $y = '', $returnstring = false)
	{
		...
		if (($s_x == 0) or ( $s_y == 0)) {
			throw new \Dinamiko\DKPDF\Vendor\Mpdf\MpdfException('Please do not use values equal to zero for scaling');
		}
	...
	function AdjustHTML($html, $tabSpaces = 8)
	{
		...
		if (0 >= (int) $limit) {
			throw new \Dinamiko\DKPDF\Vendor\Mpdf\MpdfException(sprintf(
	...
	function OverWrite($file_in, $search, $replacement, $dest = Destination::DOWNLOAD, $file_out = "mpdf")
	{
		...
		switch ($dest) {
			...
			case Destination::FILE:
				...
				if (!$f) {
					throw new \Dinamiko\DKPDF\Vendor\Mpdf\MpdfException('Unable to create output file: ' . $file_out);
				}
