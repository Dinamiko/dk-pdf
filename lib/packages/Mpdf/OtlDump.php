...
class OtlDump
{
	...
	function getMetrics($file, $fontkey, $TTCfontID = 0, $debug = false, $BMPonly = false, $kerninfo = false, $useOTL = 0, $mode = null)
	{
		...
		if ($version == 0x74746366 && !$TTCfontID) {
			throw new \Dinamiko\DKPDF\Vendor\Mpdf\Exception\FontException("TTCfontID for a TrueType Collection has to be defined in ttfontdata configuration key (" . $file . ")");
		}

		if (!in_array($version, [0x00010000, 0x74727565]) && !$TTCfontID) {
			throw new \Dinamiko\DKPDF\Vendor\Mpdf\Exception\FontException("Not a TrueType font: version=" . $version);
		}

		if ($TTCfontID > 0) {
			$this->version = $version = $this->read_ulong(); // TTC Header version now
			if (!in_array($version, [0x00010000, 0x00020000])) {
				throw new \Dinamiko\DKPDF\Vendor\Mpdf\Exception\FontException("Error parsing TrueType Collection: version=" . $version . " - " . $file);
			}
	...
	function extractInfo($debug = false, $BMPonly = false, $kerninfo = false, $useOTL = 0)
	{
		...
		if ($format != 0 && $format != 1) {
			throw new \Dinamiko\DKPDF\Vendor\Mpdf\Exception\FontException("Error loading font: Unknown name table format " . $format);
		}
		...
		for ($i = 0; $i < $numRecords; $i++) {
			...
			if ($platformId == 3 && $encodingId == 1 && $languageId == 0x409) { // Microsoft, Unicode, US English, PS Name
				...
				if ($length % 2 != 0) {
					throw new \Dinamiko\DKPDF\Vendor\Mpdf\Exception\FontException("Error loading font: PostScript name is UTF-16BE string of odd length");
				}
				$length /= 2;
				$N = '';
				while ($length > 0) {
		...
		if ($glyphDataFormat != 0) {
			throw new \Dinamiko\DKPDF\Vendor\Mpdf\Exception\FontException('Error loading font: Unknown glyph data format ' . $glyphDataFormat);
		}
		...
		if (isset($this->tables["OS/2"])) {
			...
			if ($fsType == 0x0002 || ($fsType & 0x0300) != 0) {
				...
				if (!$overrideTTFFontRestriction) {
					throw new \Dinamiko\DKPDF\Vendor\Mpdf\Exception\FontException('Font file ' . $this->filename . ' cannot be embedded due to copyright restrictions.');
				}
		...
		if ($debug) {
			...
			if ($ver_maj < 1 || $ver_maj > 4) {
				throw new \Dinamiko\DKPDF\Vendor\Mpdf\Exception\FontException('Error loading font: Unknown post table version ' . $ver_maj);
			}
		} else {
			$this->skip(4);
		}
		...
		if ($metricDataFormat != 0) {
			throw new \Dinamiko\DKPDF\Vendor\Mpdf\Exception\FontException('Error loading font: Unknown horizontal metric data format ' . $metricDataFormat);
		}
		...
		if ($debug) {
			...
			$ver_min = $this->read_ushort();
			if ($ver_maj != 1) {
				throw new \Dinamiko\DKPDF\Vendor\Mpdf\Exception\FontException('Error loading font: Unknown maxp table version ' . $ver_maj);
			}
		} else {
			$this->skip(4);
		}
		...
		if (!$unicode_cmap_offset) {
			throw new \Dinamiko\DKPDF\Vendor\Mpdf\Exception\FontException('Font (' . $this->filename . ') does not have cmap for Unicode (platform 3, encoding 1, format 4, or platform 0, any encoding, format 4)');
		}
								...
								if ($Lookup[$i]['Type'] == 4) {
									...
									if ($Lookup[$i]['Type'] == 5) {
										...
										if ($Lookup[$i]['Type'] == 6) {
											...
											throw new \Dinamiko\DKPDF\Vendor\Mpdf\Exception\FontException("Lookup Type " . $Lookup[$i]['Type'] . " not supported.");
										}
									}
								}
							}
...
//print_r($Lookup); exit;
	...
	function _getGSUBignoreString($flag, $MarkFilteringSet)
	{
		...
		// Flag & 0x0010 = UseMarkFilteringSet
		if ($flag & 0x0010) {
			throw new \Dinamiko\DKPDF\Vendor\Mpdf\Exception\FontException("This font " . $this->fontkey . " contains MarkGlyphSets");
			$str = "Mark Glyph Set: ";
								...
								////////////////////////////////////////////////////////////////////////////////
									...
									////////////////////////////////////////////////////////////////////////////////
										...
										if ($Lookup[$luli]['Type'] == 7) {
											...
											//===========
											if ($PosFormat == 1) {
												throw new \Dinamiko\DKPDF\Vendor\Mpdf\Exception\FontException("GPOS Lookup Type " . $Type . " Format " . $PosFormat . " not YET TESTED.");
											} //===========
											// Format 2:
											//===========
											else {
												...
												//===========
													...
													if ($PosFormat == 3) {
														...
														throw new \Dinamiko\DKPDF\Vendor\Mpdf\Exception\FontException("GPOS Lookup Type " . $Type . ", Format " . $PosFormat . " not supported.");
													}
												}
											}
										} ////////////////////////////////////////////////////////////////////////////////
										...
										////////////////////////////////////////////////////////////////////////////////
											...
											if ($Lookup[$luli]['Type'] == 8) {
												...
												//===========
												if ($PosFormat == 1) {
													throw new \Dinamiko\DKPDF\Vendor\Mpdf\Exception\FontException("GPOS Lookup Type " . $Type . " Format " . $PosFormat . " not TESTED YET.");
												} //===========
												// Format 2:
												//===========
												else {
