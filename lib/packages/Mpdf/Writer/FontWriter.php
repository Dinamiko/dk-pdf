...
use Dinamiko\DKPDF\Vendor\Mpdf\TTFontFile;
	...
	public function writeFonts()
	{
		...
		foreach ($this->mpdf->fonts as $k => $font) {
			...
			if ($type === 'Type0') {  // Adobe CJK Fonts
				...
				if ($this->mpdf->PDFA || $this->mpdf->PDFX) {
					throw new \Dinamiko\DKPDF\Vendor\Mpdf\MpdfException('Core fonts are not allowed in PDF/A1-b or PDFX/1-a files (Times, Helvetica, Courier etc.)');
				}
				...
				throw new \Dinamiko\DKPDF\Vendor\Mpdf\MpdfException(sprintf('Unsupported font type: %s (%s)', $type, $name));
			}
		}
	}
