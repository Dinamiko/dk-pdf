		...
		foreach ($this->mpdf->images as $file => $info) {
			...
			if (isset($info['icc']) && ( $info['icc'] !== false)) {
				...
				if ($this->mpdf->PDFX || ($this->mpdf->PDFA && $this->mpdf->restrictColorSpace === 3)) {
					throw new \Dinamiko\DKPDF\Vendor\Mpdf\MpdfException('PDFA1-b and PDFX/1-a files do not permit using mixed colour space (' . $file . ').');
				}
				...
				if ($info['cs'] === 'DeviceCMYK') {
					if ($this->mpdf->PDFA && $this->mpdf->restrictColorSpace !== 3) {
						throw new \Dinamiko\DKPDF\Vendor\Mpdf\MpdfException('PDFA1-b does not permit Images using mixed colour space (' . $file . ').');
					}
					if ($info['type'] === 'jpg') {
