...
use Dinamiko\DKPDF\Vendor\Mpdf\Utils\PdfDate;

use Dinamiko\DKPDF\Vendor\Psr\Log\LoggerInterface;

class MetadataWriter implements \Dinamiko\DKPDF\Vendor\Psr\Log\LoggerAwareInterface
{
	  ...
	  <rdf:li>' . $this->mpdf->author . '</rdf:li>
		...
		elseif ($this->mpdf->PDFA) {
			...
			if (strpos($this->mpdf->PDFAversion, '-') === false) {
				throw new \Dinamiko\DKPDF\Vendor\Mpdf\MpdfException(sprintf('PDFA version (%s) is not valid. (Use: 1-B, 3-B, etc.)', $this->mpdf->PDFAversion));
			}
	...
	public function writeOutputIntent() // _putoutputintent
	{
		...
		if ($this->mpdf->ICCProfile) {
			if (!file_exists($this->mpdf->ICCProfile)) {
				throw new \Dinamiko\DKPDF\Vendor\Mpdf\MpdfException(sprintf('Unable to find ICC profile "%s"', $this->mpdf->ICCProfile));
			}
	...
	public function writeAssociatedFiles() // _putAssociatedFiles
	{
		if (!function_exists('gzcompress')) {
			throw new \Dinamiko\DKPDF\Vendor\Mpdf\MpdfException('ext-zlib is required for compression of associated files');
		}
		...
		foreach ($this->mpdf->associatedFiles as $k => $file) {
			...
			if (!$fileContent) {
				throw new \Dinamiko\DKPDF\Vendor\Mpdf\MpdfException(sprintf('Cannot access associated file - %s', $file['path']));
			}
				...
				if (isset($this->mpdf->PageAnnots[$n])) {
					...
					foreach ($this->mpdf->PageAnnots[$n] as $key => $pl) {
						...
						if ($fileAttachment) {
							...
							if (!$file) {
								throw new \Dinamiko\DKPDF\Vendor\Mpdf\MpdfException('mPDF Error: Cannot access file attachment - ' . $pl['opt']['file']);
							}
