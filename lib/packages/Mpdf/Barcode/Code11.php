 ...
 */
class Code11 extends \Dinamiko\DKPDF\Vendor\Mpdf\Barcode\AbstractBarcode implements \Dinamiko\DKPDF\Vendor\Mpdf\Barcode\BarcodeInterface
{
	...
	private function init($code, $printRatio)
	{
		...
		for ($i = 0; $i < $len; ++$i) {
			...
			if (!isset($chr[$code[$i]])) {
				throw new \Dinamiko\DKPDF\Vendor\Mpdf\Barcode\BarcodeException(sprintf('Invalid character "%s" in CODE11 barcode value "%s"', $code[$i], $code));
			}
