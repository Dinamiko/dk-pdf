 ...
 */
class S25 extends \Dinamiko\DKPDF\Vendor\Mpdf\Barcode\AbstractBarcode implements \Dinamiko\DKPDF\Vendor\Mpdf\Barcode\BarcodeInterface
{
	...
	private function init($code, $checksum)
	{
		...
		for ($i = 0; $i < $clen; ++$i) {
			...
			if (!isset($chr[$digit])) {
				// invalid character
				throw new \Dinamiko\DKPDF\Vendor\Mpdf\Barcode\BarcodeException(sprintf('Invalid character "%s" in S25 barcode value "%s"', $digit, $code));
			}
			$seq .= $chr[$digit];
		}
