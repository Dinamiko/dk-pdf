 ...
 */
class I25 extends \Dinamiko\DKPDF\Vendor\Mpdf\Barcode\AbstractBarcode implements \Dinamiko\DKPDF\Vendor\Mpdf\Barcode\BarcodeInterface
{
	...
	private function init($code, $printRatio, $checksum)
	{
		...
		for ($i = 0; $i < $clen; $i = ($i + 2)) {
			...
			if ((!isset($chr[$charBar])) or (!isset($chr[$charSpace]))) {
				// invalid character
				throw new \Dinamiko\DKPDF\Vendor\Mpdf\Barcode\BarcodeException(sprintf('Invalid I25 barcode value "%s"', $code));
			}
