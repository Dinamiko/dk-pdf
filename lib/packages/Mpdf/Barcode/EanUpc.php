 ...
 */
class EanUpc extends \Dinamiko\DKPDF\Vendor\Mpdf\Barcode\AbstractBarcode implements \Dinamiko\DKPDF\Vendor\Mpdf\Barcode\BarcodeInterface
{
	...
	private function init($code, $length)
	{
		if (preg_match('/[\D]+/', $code)) {
			throw new \Dinamiko\DKPDF\Vendor\Mpdf\Barcode\BarcodeException(sprintf('Invalid EAN UPC barcode value "%s"', $code));
		}
		...
		if ($upce) {
			...
			if ($invalidUpce) {
				throw new \Dinamiko\DKPDF\Vendor\Mpdf\Barcode\BarcodeException('UPC-A cannot produce a valid UPC-E barcode');
			}
		}
