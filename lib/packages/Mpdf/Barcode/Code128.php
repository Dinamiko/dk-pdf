...
namespace Dinamiko\DKPDF\Vendor\Mpdf\Barcode;
 ...
 */
class Code128 extends \Dinamiko\DKPDF\Vendor\Mpdf\Barcode\AbstractBarcode implements \Dinamiko\DKPDF\Vendor\Mpdf\Barcode\BarcodeInterface
{
		...
		switch (strtoupper($type)) {
			...
			case 'RAW':
				...
				foreach (explode(" ", $code) as $v) {
					...
					if (is_numeric($v) && round($v, 0) == $v) {
						...
						if ($v>=0 && $v<=105) {
							...
							throw new \Dinamiko\DKPDF\Vendor\Mpdf\Barcode\BarcodeException(sprintf('Invalid CODE128RAW barcode value "%s". 0-105 needed', $code));
						}
				...
				if ($startid < 103 || $startid > 105) {
					throw new \Dinamiko\DKPDF\Vendor\Mpdf\Barcode\BarcodeException(sprintf('Invalid CODE128RAW startid value "%s". Must be 103, 104 or 105 (for A, B or C)', $startid));
				}
			...
			case 'C':
				...
				if ((strlen($code) % 2) != 0) {
					...
					throw new \Dinamiko\DKPDF\Vendor\Mpdf\Barcode\BarcodeException('Invalid CODE128C barcode value');
				}
				...
				for ($i = 0; $i < $hclen; ++$i) {
					if ($code[2 * $i]<"0" || $code[2 * $i]>"9" || $code[2 * $i + 1]<"0" || $code[2 * $i + 1]>"9") {
						throw new \Dinamiko\DKPDF\Vendor\Mpdf\Barcode\BarcodeException(sprintf('Invalid character "%s" in CODE128C barcode value "%s"', $code[$i], $code));
					}
			...
			default:
				throw new \Dinamiko\DKPDF\Vendor\Mpdf\Barcode\BarcodeException(sprintf('Invalid CODE128 barcode type "%s"', $type));
		}
		...
		for ($i = 0; $i < $clen; ++$i) {
			if ($ean && $i == 0) {
				...
				if (strpos($keys, $code[$i]) === false) {
					throw new \Dinamiko\DKPDF\Vendor\Mpdf\Barcode\BarcodeException(sprintf('Invalid character "%s" in CODE128%s barcode value "%s"', $code[$i], $type, $code));
				}
