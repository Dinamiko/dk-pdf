 ...
 */
class Code93 extends \Dinamiko\DKPDF\Vendor\Mpdf\Barcode\AbstractBarcode implements \Dinamiko\DKPDF\Vendor\Mpdf\Barcode\BarcodeInterface
{
	...
	private function init($code)
	{
		...
		for ($i = 0; $i < $clen; ++$i) {
			if (ord($code[$i]) > 127) {
				throw new \Dinamiko\DKPDF\Vendor\Mpdf\Barcode\BarcodeException(sprintf('Invalid character "%s" in CODE93 barcode value "%s"', $code[$i], $code));
			}
		...
		for ($i = 0; $i < $clen; ++$i) {
			...
			if (!isset($chr[$char])) {
				// invalid character
				throw new \Dinamiko\DKPDF\Vendor\Mpdf\Barcode\BarcodeException(sprintf('Invalid CODE93 barcode value "%s"', $code));
			}
			for ($j = 0; $j < 6; ++$j) {
				if (($j % 2) == 0) {
					$t = true; // bar
