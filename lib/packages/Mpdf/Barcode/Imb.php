 ...
 */
class Imb extends \Dinamiko\DKPDF\Vendor\Mpdf\Barcode\AbstractBarcode implements \Dinamiko\DKPDF\Vendor\Mpdf\Barcode\BarcodeInterface
{
	...
	public function __construct($code, $xDim, $gapWidth, $daft)
	{
		if (!function_exists('bcadd')) {
			throw new \Dinamiko\DKPDF\Vendor\Mpdf\Barcode\BarcodeException('IMB barcodes require bcmath extension to be loaded.');
		}
	...
	private function init($code, $gapWidth, $daft)
	{
		...
		switch (strlen($routingCode)) {
			...
			case 11:
				...
				break;
			default:
				throw new \Dinamiko\DKPDF\Vendor\Mpdf\Barcode\BarcodeException(sprintf('Invalid MSI routing code "%s"', $routingCode));
		}
