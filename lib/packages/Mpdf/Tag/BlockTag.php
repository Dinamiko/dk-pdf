...
use Dinamiko\DKPDF\Vendor\Mpdf\Conversion\DecToAlpha;
use Dinamiko\DKPDF\Vendor\Mpdf\Conversion\DecToRoman;
	...
	public function open($attr, &$ahtml, &$ihtml)
	{
		...
		if ((isset($p['POSITION'])
			...
			if ($this->mpdf->inFixedPosBlock) {
				throw new \Dinamiko\DKPDF\Vendor\Mpdf\MpdfException('Cannot nest block with position:fixed or position:absolute');
			}
		...
		if ($currblk['inner_width'] < $mw) {
			...
			// if ($currblk['inner_width'] < $mw) { throw new \Dinamiko\DKPDF\Vendor\Mpdf\MpdfException("DIV is too narrow for text to fit!"); }
		}
